...
CREATE TEMPORARY TABLE tmp_backup(`id`,`name`,`description`,`tcategories_id`);;
CREATE TABLE `tnotes` ( `id` INTEGER PRIMARY KEY AUTOINCREMENT  ,`name` TEXT ,`description` TEXT ,`tcategories_id` INTEGER    );;
CREATE INDEX index_foreignkey_tnotes_tcategories ON `tnotes` (tcategories_id) ;
CREATE TEMPORARY TABLE tmp_backup(`id`,`name`,`description`,`tcategories_id`);;
CREATE TABLE `tnotes` ( `id` INTEGER PRIMARY KEY AUTOINCREMENT  ,`name` TEXT ,`description` TEXT ,`tcategories_id` INTEGER   , FOREIGN KEY(`tcategories_id`)
						 REFERENCES `tcategories`(`id`)
						 ON DELETE SET NULL ON UPDATE SET NULL );;
CREATE INDEX index_foreignkey_tnotes_tcategories ON `tnotes` (tcategories_id) ;
