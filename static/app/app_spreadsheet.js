...
export function fnCreateSpeadsheet(sID)
{
    ...
    //     var oData = Papa.parse(paste)
    //     var aData = oData.data
    //     var oD = { rows: {} }

    //     for (var iR in aData) {
    //         var aRow = aData[iR]
    //         if (!oD.rows[iR]) oD.rows[iR] = { cells: {} }
    //         for (var iC in aRow) {
    //             if (!oD.rows[iR].cells[iC]) oD.rows[iR].cells[iC] = { text: '' }
    //             oD.rows[iR].cells[iC].text = aRow[iC]
    //         }
    //     }
